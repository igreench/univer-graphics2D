...
void MainWindow::loadConfig() {
    ...
    for (int i = 0; i < nodeList.count(); i++) {
        ...
        qDebug() << "comment: " << node.text();
    }

    nodeList = docElem.elementsByTagName("Point");
    for (int i = 0; i < nodeList.count(); i++) {
        QDomElement node = nodeList.at(i).toElement();
        qDebug() << "x: " << node.attribute("x").toInt() << " y: " << node.attribute("y").toInt();
        ui->widget->addPoint(QPoint(node.attribute("x").toInt(), node.attribute("y").toInt()));
    }

    nodeList = docElem.elementsByTagName("entry");
    for (int i = 0; i < nodeList.count(); i++) {
        QDomElement node = nodeList.at(i).toElement();
        if ("PanelWidth" == node.attribute("key")) {
            qDebug() << "PanelWidth:" << node.text();
        }

        if ("PanelHeight" == node.attribute("key")) {
